...
function QuadMesh(nameIn, sceneNameIn)
{
    ...
    this.GenerateNormalCoords = function( vertNormals, faces, positionCoords )
    {
        ...
        for(var i=0; i<faces.length; ++i)
        {
            ...
            if(numVertIdxs < 3)
                DPrintf("GenerateNormalCoords: expected 3 or more vertIdx's, got: " + numVertIdxs);
            //newell's method
            var normal = [0,0,0];
        ...
        for(var i=0; i<faces.length; ++i)
        {
            ...
            if( vertsSize == 3) //triangle
            {
                for(var j=0; j<3*graphics.uvCard; j+=graphics.uvCard)
                {
                    ...
                    uvCoords[cI++] = 1-faces[i].uvs[j+1];
                }
            }
            else if(vertsSize == 4) //quad. tesselate into two triangles
            {
                uvCoords[cI++] = faces[i].uvs[0*graphics.uvCard+0];
                uvCoords[cI++] = 1-faces[i].uvs[0*graphics.uvCard+1];
                
                uvCoords[cI++] = faces[i].uvs[1*graphics.uvCard+0];
                uvCoords[cI++] = 1-faces[i].uvs[1*graphics.uvCard+1];
                
                uvCoords[cI++] = faces[i].uvs[2*graphics.uvCard+0];
                uvCoords[cI++] = 1-faces[i].uvs[2*graphics.uvCard+1];
                
                uvCoords[cI++] = faces[i].uvs[2*graphics.uvCard+0];
                uvCoords[cI++] = 1-faces[i].uvs[2*graphics.uvCard+1];
                
                uvCoords[cI++] = faces[i].uvs[3*graphics.uvCard+0];
                uvCoords[cI++] = 1-faces[i].uvs[3*graphics.uvCard+1];
    ...
    this.Draw = function(verts, normals, uvs)
    {
        ...
        this.tesselateCoords(normals, this.faces, this.vertNormals);
    ...
    if( this.shaderNames.length < 1 ){
        DPrintf('QuadMesh: ' + this.meshName + ', failed to read any materials, loading default material');
